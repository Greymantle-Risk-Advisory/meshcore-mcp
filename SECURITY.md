...
| CI-driven deploy leaking the Cloudflare API token to a malicious PR (this is a public repo — anyone can open one) | `CLOUDFLARE_API_TOKEN` is a repo secret, scoped to _Edit Cloudflare Workers_ only (not the global API key); the deploy job runs only on `push` to `main`, never on `pull_request` — GitHub Actions doesn't expose secrets to fork PRs by default, and this is a second, explicit gate on top of that | `.github/workflows/ci.yml` (`deploy` job's `if:`)                 |
