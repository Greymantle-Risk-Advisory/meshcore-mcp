...
Every endpoint this server wraps is unauthenticated and public on the
upstream CoreScope instance itself, whether it's a `GET` (`/api/stats`,
`/api/nodes`, `/api/observers`, `/api/analytics/*`) or one of three `POST`
routes (`/api/packets/observations`, `/api/decode`, `/api/paths/inspect`).
Those three are `POST` because they take a JSON request body, not because
they change anything — `POST` and "writes data" aren't the same thing, and
conflating them was worth double-checking rather than assuming. Each
handler's actual implementation was read (not just its route registration)
to confirm none of the three touches the database or any other state
before it was wrapped here.

Adding auth here wouldn't protect anything that isn't already open — it
would just add complexity for no security benefit. CoreScope's real
write/admin routes (`PUT /api/config/geo-filter`, `POST /api/packets`,
`POST /api/admin/*`, `POST /api/perf/reset`) require an API key on the
upstream and are never touched by this server; see `src/corescope.ts`,
whose `postJSON` helper is only ever pointed at the three read-only POST
routes above, never the gated write ones.
...
| Oversized POST payloads (e.g. thousands of hashes/prefixes in one call) wasting compute on a public, authless endpoint                                                                                                                  | Client-side bounds mirror upstream's own limits (200 hashes, 64 prefixes) at two layers: the MCP tool's zod schema rejects an oversized array before the tool body even runs, and the CoreScope client rejects it again as defense in depth                                                                                                                                                                                                                                   | `src/index.ts` (tool `inputSchema`s), `src/corescope.ts` (`maxObservationHashes`, `maxPathInspectPrefixes`) |
...
| GitHub Actions expression injection: `${{ vars.X }}` interpolated directly into a `run:` shell command is substituted as raw text _before_ the shell runs, so a value containing shell metacharacters becomes executable code, not data | `CORESCOPE_BASE_URL`/`MESH_NAME` are bound via a step-level `env:` block and referenced as `$CORESCOPE_BASE_URL`/`$MESH_NAME` inside `run:`, never interpolated directly. Only repo admins can set these vars today, so the practical exposure was already low, but routing through `env:` removes the class of bug rather than relying on that                                                                                                                               | `.github/workflows/ci.yml` (`deploy` job)                                                                   |
